				...
				$http.get(options.uri_base + options.uri_point + access_token + options.filters + options.sub_filter + (i+1) ).
	    			success(function(res) {
						for (var j =  0; j <= res.data.length - 1; j++) {
							...
							$http.get(options.uri_base + options.uri_base_op + res.data[j].opportunity.id + options.op_manager_query + access_token).
							success(
								function (r) {
								opManFullName = r.managers[0].full_name;
								opManEmail = r.managers[0].email;
							...
							people_expa.push({
								...
								"op_manager_name": opManFullName,
								"op_manager_email": opManEmail,
