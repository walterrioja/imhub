...
app.controller('Analytics', ['$scope', '$http', function ($scope,$http) {
	$scope.go = function() {
		...
		//iGV
 		if(programa == '1'){
 			options.filters = '&filters%5Bdate_approved%5Bfrom%5D%5D=' + start_date + '&filters%5Bdate_approved%5Bto%5D%5D=' + end_date + '&filters%5Bopportunity_committee%5D=1553&filters%5Bprogrammes%5D%5B%5D=1&per_page=200&sort=status'
		}//iGT
		if(programa == '2'){
			options.filters = '&filters%5Bdate_approved%5Bfrom%5D%5D=' + start_date + '&filters%5Bdate_approved%5Bto%5D%5D=' + end_date + '&filters%5Bopportunity_committee%5D=1553&filters%5Bprogrammes%5D%5B%5D=2&per_page=200&sort=status'
		}//iGE
                if(programa == '5'){
                        options.filters = '&filters%5Bdate_approved%5Bfrom%5D%5D=' + start_date + '&filters%5Bdate_approved%5Bto%5D%5D=' + end_date + '&filters%5$Bopportunity_committee%5D=1553&filters%5Bprogrammes%5D%5B%5D=5&per_page=200&sort=status'
                }
				...
				$http.get(options.uri_base + options.uri_point + access_token + options.filters + options.sub_filter + (i+1) ).
	    			success(function(res) {
						for (var j =  0; j <= res.data.length - 1; j++) {
							var lc;
							if(programa == '1' || programa == '2' || programa'5'){
